...
test.describe('Add to cart', () => {
  test('adds a single product from the Books category and updates the mini-cart count', async ({ page, homePage }) => {
  ...
  test('adds a product and verifies it appears in the cart page with the correct name', async ({ page, homePage, cartPage }) => {
    await homePage.openCategory('books');
  ...
  test('adding two different products results in two cart line items', async ({ page, homePage, cartPage }) => {
    await homePage.openCategory('books');
  ...
  test('removing an item from the cart empties it', async ({ page, homePage, cartPage }) => {
    await homePage.openCategory('books');
    await homePage.addToCartButtons.first().click();
    await expect(page.locator('#bar-notification.success')).toBeVisible({ timeout: 10000 });
